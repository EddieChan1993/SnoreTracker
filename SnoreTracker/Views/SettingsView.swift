...
struct SettingsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // ── 主题切换 ──
                    settingCard(title: "外观主题", icon: "paintpalette.fill") {
                        VStack(spacing: 12) {
                            ForEach(AppTheme.all) { t in
                                ThemeOptionRow(theme: t, isSelected: themeManager.selectedThemeID == t.id) {
                    ...
                    settingCard(title: "检测灵敏度", icon: "ear.fill") {
                        VStack(spacing: 16) {
                            sliderRow(label: "音量灵敏度", value: $minimumRMS,
                                      range: 0.01...0.10,
                                      leftLabel: "高", rightLabel: "低",
                                      display: sensitivityLabel,
                                      hint: "手机离头部越远，调越高（默认：中）") {
                                sessionManager.audioService.minimumRMS = Float(minimumRMS)
                            }
                            Divider().background(Color.white.opacity(0.1))
                            sliderRow(label: "频率匹配度", value: $snoreScore,
                                      range: 0.20...0.70,
                                      leftLabel: "宽松", rightLabel: "严格",
                            ...
                            Divider().background(Color.white.opacity(0.1))
                            sliderRow(label: "确认延迟", value: $confirmDelay,
                                      range: 0.3...3.0,
                                      leftLabel: "快", rightLabel: "慢",
                                      display: String(format: "%.1fs", confirmDelay),
                                      hint: "持续多少秒才算一次呼噜") {
                                sessionManager.audioService.confirmDelay = confirmDelay
                            }
                            Divider().background(Color.white.opacity(0.1))
                            sliderRow(label: "停止延迟", value: $silenceDelay,
                                      range: 2.0...15.0,
                                      leftLabel: "短", rightLabel: "长",
                    ...
                    settingCard(title: "关于", icon: "info.circle.fill") {
                        VStack(spacing: 12) {
                            ...
                            Divider().background(Color.white.opacity(0.1))
                            dataRow(label: "检测算法", value: "FFT 频率分析")
                            Divider().background(Color.white.opacity(0.1))
                            HStack {
        ...
        .confirmationDialog("确认清除", isPresented: $showClearConfirm, titleVisibility: .visible) {
            Button("清除所有数据", role: .destructive) { sessionManager.clearAllData() }
    ...
    private func sliderRow(label: String, value: Binding<Double>, range: ClosedRange<Double>,
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !hint.isEmpty {
                Text(hint).font(.system(size: 11)).foregroundColor(.white.opacity(0.3))
            }
        }
    }
    ...
    private var totalRecordingCount: Int { store.sessions.reduce(0) { $0 + $1.snoringEvents.count } }
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(theme.displayName)
                        ...
                        .foregroundColor(.white)
                    HStack(spacing: 6) {
                        let dots = [theme.accent, theme.accentLight, theme.snoringAccent]
